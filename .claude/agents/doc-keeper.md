...
| `docs/developer/architecture/web-ui.md` | The "NiceGUI web UI foundations" design decisions — `host/web/`'s own architecture. |
...
| `docs/developer/modules/core.md` | Per-module reference for `config/`, `server/`, and `host/`'s non-web modules (key types/functions, design notes). No `(~NNN lines)` annotations — see Hard boundaries. |
| `docs/developer/modules/web-ui.md` | Per-module reference for `host/web/*.py`. Same annotation rule as above. |
...
- **No approximate line-count annotations.** Never add or maintain per-module `(~NNN lines)`-style annotations (these have appeared in `docs/developer/modules/`); they are perpetually stale and create recurring drift noise. When you edit a section that carries such an annotation, strip it rather than correcting the number.
...
`docs/developer/modules/` and `docs/developer/architecture/` are each split into a `core.md`
and a `web-ui.md` page (several hundred to ~850 lines apiece). Reading one whole, every run,
is the single biggest cost of this agent.
...
- **New component, changed responsibility, or new data-flow step** → `docs/developer/architecture/core.md`, or `architecture/web-ui.md` if it's `host/web/`-specific.
...
- **Profile schema / domain-profile behaviour** → `architecture/core.md` (and README if user-facing).

...
**Key class:** `Settings` — a `BaseSettings` subclass with fields for LLM endpoint, safety, domain profile, document memory, and egress settings. `llm_base_url` and `llm_api_key` default to `""` so `Settings()` can be instantiated before the wizard runs. `target_dir: Path | None = None` holds the directory being organized this run — populated from a `TARGET_DIR` env var set by the host (`mcp_session`) when it launches the server subprocess; `None` outside a real run (e.g. some test harnesses), in which case path-confinement guards fall back to just the server's own working directory (M2). `sessions_dir: Path = Path(".organizer/sessions")` (Y2) is where per-session transcript/LLM-history snapshots live, inside the same `.organizer/` allowlist/egress boundary the registry/journal/graph already trust — deliberately separate from the home-directory session *index* (metadata only), which lives outside that boundary; see `user_sessions_index_path()` below. `memory_path: Path = Path(".organizer/memory.md")` (Z5) is the persistent per-directory memory file — read on every fresh run (`host/agent.py`'s `_load_memory`) and, via the plan-gated `memory_note` op, appendable by the agent; a `Path` field, so it IS rebased by `for_target()` below (unlike `analyzer_batch_size`). `analyzer_batch_size: int = Field(default=10, ge=1, le=50)` (Z3) is how many documents the stateless ANALYZE step sends to the LLM per batch — user-settable per run via the organize starter pane's "Documents per analysis batch" field (`host/web/main.py`), applied as a run-scoped override by `AgentBridge._load_settings()` (`host/web/bridge.py`). A plain scalar, not a `Path` field, so `for_target()` (below) does not rebase it. `effective_allowlist_dirs() -> list[Path]` (M7) returns `allowlist_dirs` unchanged if it's non-empty — an explicit operator config always wins outright, never merged with `target_dir` — otherwise defaults to `[target_dir]` if set, else `[]`; `server/main.py`'s `read_file`, `extract_text`, `compare_documents`, and the batch forms `read_file_batch`/`extract_text_batch` (O1) call this instead of the raw `allowlist_dirs` field. `for_target(target: Path) -> Settings` (P2, per-directory memory) returns a copy with `quarantine_dir`/`journal_path`/`events_path`/`plans_dir`/`registry_path`/`graph_path`/`archive_path`/`egress_path`/`token_log_path`/`llm_debug_log_path`/`sessions_dir`/`memory_path` (Y2; `memory_path` Z5) rebased onto `target.resolve()` when they're relative (an already-absolute override passes through unchanged), so a run's memory lives inside the directory being organized rather than telcontar's project root; `profiles_dir` is deliberately left untouched. `load()` calls `for_target(settings.target_dir)` itself whenever `target_dir` is set.
...
| `save_user_config(updates: dict[str, str]) -> None` | Writes non-sensitive keys to `~/.telcontar/config.env`; stores the API key in the OS keyring (falls back to the config file if keyring is unavailable). As its last statement, bumps the module-global config revision counter (Z2) — never on a `PlaintextKeyFallbackNeeded` raise, since that happens before anything is written. |
| `read_user_config() -> dict[str, str]` | Returns the raw key→value pairs from `~/.telcontar/config.env` (lowercase keys, no API key). |
| `config_revision() -> int` (Z2) | Returns the current value of a module-global counter bumped once per successful `save_user_config()` call. `host/llm.py`'s `ReloadingClient` polls this to detect a mid-session config change and reload without a full telcontar restart; process-lifetime, so tests must assert a delta, never an absolute value. |
| `user_sessions_index_path() -> Path` (Y2) | Returns `_USER_CONFIG_DIR / "sessions.json"` — the home-directory session index (metadata only: `run_id`/`target`/`mode`/timestamps/`status`, never corpus-derived text). Derived at call time from the module-global `_USER_CONFIG_DIR` rather than a frozen constant, so a test that patches it (the same isolation `tests/test_settings.py` already relies on) redirects this too. Consumed by `host/web/sessions.py`, never called from `server/`. |
...
| Plan-building | `propose_rename`, `propose_move`, `propose_quarantine`, `propose_create_file`, `propose_update_file`, `propose_create_dir`, `propose_archive_document`, `propose_compress_quarantine`, `propose_memory_note` (Z5) |
...
| Persistent memory (not MCP tools) | `append_memory_note`, `_undo_memory_note` (Z5) — called by `execute_plan`'s and `undo_last`'s `memory_note` branches respectively, never registered as MCP tools themselves |
...
**Design note (Y6, GH #57):** `execute_plan` gained two optional params, `target_dir: Path | None = None` and `empty_folder_policy: str = "quarantine"` (server-supplied via `cfg.target_dir`/`cfg.empty_folder_policy`, never model-facing — the MCP-exposed signature is still just `execute_plan(plan_id: str)`). When `target_dir` is set, after the op loop (and only on a non-hard-stop run) `execute_plan` calls `_sweep_emptied_dirs` on the parent directories of every completed `move`/`quarantine`/`archive_document` op, disposing of any that are now actually empty. The return dict gains an always-present `emptied_folders` key (`[]` when nothing was swept, including on the hard-stop early return).

- `_sweep_emptied_dirs(candidates, *, target_dir, quarantine_dir, empty_folder_policy, plan_created_dirs, journal_path, plan_id) -> list[dict]` (Y6) — disposes of each candidate directory left empty by this run's moves, deepest-first (sorted by path-part count), re-queueing a disposed folder's parent so a multi-level hollow shell collapses fully. Returns `[]` immediately if `empty_folder_policy == "off"`. Skips (silently, no result entry) a candidate that no longer exists, is `is_sweep_protected`, or is not `is_dir_empty`. An eligible folder is quarantined (`safe_quarantine_dir_path` + `shutil.move`, falling back to an `empty_marker_path` in-place rename on any `OSError`) or always in-place renamed, per `empty_folder_policy`; either way the disposal is journaled immediately under the existing `"quarantine"`/`"rename"` `op_type`, with `target_kind: "dir"` and `reason: "emptied_by_plan"` added. Returns one `{"path", "action", "dst", "error"}` dict per folder processed — `action` is `"quarantined"`, `"renamed"`, or `"skipped"` (an `OSError` during disposal, recorded with `error` set, but never re-raised — a sweep failure never fails the plan).
...
**Design note (Z5, #64):** `review_plan` exempts `memory_note` ops from both its duplicate-op and missing-source checks — like `create_dir`, a note's `src` (the memory file) legitimately doesn't exist yet on the first-ever note, and unlike every other op type, several notes legitimately share one `src` within a plan.

- `propose_memory_note(note, plan_id, plans_dir, memory_path, max_chars) -> dict` (Z5, #64) — the only write path into the persistent per-directory memory file; stages a `memory_note` op onto a pending plan rather than writing directly. `memory_path`/`max_chars` are server-resolved from config (`cfg.memory_path`/`cfg.max_snippet_chars`), never agent-supplied. Raises `ValueError` if `note` is blank or over `_MAX_MEMORY_NOTE_CHARS = 500` chars. Rather than erroring, silently *skips* (`{"skipped": True, "reason": ...}`, no op added) when: the plan already has `_MAX_MEMORY_NOTES_PER_PLAN = 10` notes staged, an equivalent note (via `server.memory.contains_note`'s case/whitespace-insensitive check) is already staged in this plan or already recorded in the memory file, or the memory file is already at `max_chars` (writing further would be invisible to a future run, since `_load_memory`/`read_memory` head-truncates at that same cap)
- `append_memory_note(memory_path, note, journal_path) -> dict` (Z5) — `execute_plan`'s standalone, self-journaling apply function for a `memory_note` op (added to `_SELF_JOURNALING_OP_TYPES`, alongside `archive_document`/`compress_quarantine`): renders and appends one note via `server/memory.py`'s `render_block`/`append_block`, then journals `{op_type: "memory_note", src, dst: "", note, offset, bytes, timestamp}` — `offset`/`bytes` are the exact byte range appended, letting `undo_last` truncate the file back precisely
- `_undo_memory_note(entry, journal_path) -> dict` (Z5) — reverses a `memory_note` journal entry: verifies the memory file's current size still equals `entry["offset"] + entry["bytes"]` (this op's own recorded write) before calling `server/memory.py`'s `truncate_to`; if the size doesn't match — a later note was appended, or the file was hand-edited, since this op ran — refuses and leaves the journal entry in place rather than risk truncating away content this op never wrote
...
- `OpType` — `Literal["rename", "move", "quarantine", "create_file", "update_file", "create_dir", "archive_document", "compress_quarantine", "memory_note"]` (`memory_note` added Z5)
- `PlanOp` — dataclass with `op_id` (UUID), `op_type`, `src`, `dst`, `status`, `error`, `retries`, `params: dict | None` (op-specific data that doesn't fit `src`/`dst` — e.g. `{"content": ...}` for `create_file`/`update_file`, `{"reason": ...}` for `quarantine` (V10), `{"checksum": ..., "reason": ...}` for `archive_document`, `{"delete_originals": ...}` for `compress_quarantine`, `{"note": ...}` for `memory_note` (Z5))
...
| `normkey(path)` | (Y6) Case/separator-normalized comparison key (`os.path.normcase(os.path.normpath(str(path.resolve())))`) for comparing resolved paths across platforms. |
| `is_dir_empty(path)` | (Y6) True if `path` is a directory with zero entries; never raises — False if `path` doesn't exist, isn't a directory, or can't be listed. |
| `safe_quarantine_dir_path(src, quarantine_dir)` | (Y6) Like `safe_quarantine_path` but for directories: suffixes the whole basename on collision (`2024.backup` → `2024.backup_1`) instead of splitting a stem/suffix, which would mangle a dotted directory name. |
| `empty_marker_path(folder, prefix="_empty_")` | (Y6) Collision-safe in-place rename target for a folder the empty-folder sweep emptied but can't quarantine. |
| `is_sweep_protected(path, *, target_root, quarantine_dir, organizer_dir, plan_created_dirs)` | (Y6) True if `path` must never be auto-disposed of by the empty-folder sweep: the target root itself, anything outside it, `quarantine_dir` (or an ancestor/descendant of it), `organizer_dir` (likewise), or a directory the current plan run created via `create_dir` (`plan_created_dirs`, pre-normalized via `normkey`). |
...
**Role:** Persistent per-directory memory file format — `.organizer/memory.md` (`MEMORY_PATH`) (Z5). Holds notes that carry across sessions for the target directory: hand-written by the user, or appended by the agent via the plan-gated `memory_note` op (`propose_memory_note`/`execute_plan` in `server/tools.py`, above). Owns only the file format — read/render/append/truncate — never journaling or plan state; that split mirrors `server/events.py`'s relationship to the project event journal.

| Function | Description |
|---|---|
| `read_memory(path, max_chars) -> str` | Returns the file's text, head-truncated to `max_chars` with a `"[... content truncated ...]"` suffix when over the cap. `""` on a missing file or any read failure — feeds prompt composition, must never raise or block a run. |
| `render_block(note, *, first_write) -> str` | Renders one note as the text to append: a Markdown file header prepended on the very first write to a new/empty file, then always one dated, provenance-tagged line, `- [YYYY-MM-DD] (telcontar) <note>` — so a later read can tell agent-written notes from hand-written ones. |
| `append_block(path, block) -> tuple[int, int]` | Appends `block`, returning `(offset_before, bytes_written)` for the undo journal. Binary append of pre-encoded UTF-8 bytes — never text mode, which would translate `\n` → `\r\n` on Windows and desync the byte offset from what `undo_last` truncates back to. Prepends a newline to `block` (counted in `bytes_written`) if the existing file doesn't already end in one. On a partial-write `OSError`, truncates back to `offset_before` before re-raising, so `execute_plan`'s up-to-3 retries can't duplicate a partial append. |
| `truncate_to(path, offset) -> None` | Truncates `path` back to `offset` bytes — the undo primitive for a `memory_note` op, and the recovery step `append_block` itself uses on a failed write. |
| `contains_note(text, note) -> bool` | True if `note` already appears in `text`, ignoring case and whitespace differences — used at proposal time (`propose_memory_note`) to skip re-staging a note that's already recorded or already staged. |
...
- `CostApprovalCallback` — `Callable[[str, dict], Awaitable[CostApprovalResult]]`; given the summary text plus `{"new": int, "already_analyzed": int, "estimated_tokens": int, "batch_size": int}` (P8 finishes the data-shape migration P6 deferred — the dict originally carried `{"documents": int, "estimated_tokens": int}`; `batch_size` added U5 — previously omitted from `data` entirely, so both UIs' cost dialogs fell back to a hardcoded display default of 10 regardless of the configured batch size, and `host/app.py`'s call site didn't even forward the fallback through to `CostEstimateModal`; that configured value is `Settings.analyzer_batch_size` as of Z3, previously the fixed `_ANALYZER_BATCH_SIZE` constant)
...
- The stateless analyzer's (P5) batch size for NEW documents was previously a fixed module constant, `_ANALYZER_BATCH_SIZE = 10`; as of Z3 (#66) it is user-settable via `Settings.analyzer_batch_size` (`config/settings.py`, default 10, bounded 1–50) and resolved per call by `_resolve_analyzer_batch_size(settings)` — see **Key functions** below
...
- `composed_system_prompts(settings, project_root=None) -> dict[str, str]` (V11) — read-only introspection for the Settings "What telcontar tells the model" panel: renders and returns all three system prompts (`{"organize": ..., "query": ..., "analyze": ...}`) from a single profile load, reusing the same rendering steps `_build_system_prompt`/`_build_query_system_prompt`/`_analyze_batch` already use (factored into `_render_system_prompt`/`_render_query_system_prompt`/`_build_analyzer_system_prompt`) rather than duplicating them or loading the profile three times. `project_root` defaults to the exact expression `run_agent_loop` uses when omitted (`Path(__file__).resolve().parent.parent`) — load-bearing, since `_load_naming_conventions` reads `.organizer/NAMING.md` relative to the repo root, not any run's target directory, so a different default would silently display a prompt telcontar does not actually send. The ANALYZE prompt is rendered for a full `settings.analyzer_batch_size`-document batch, illustrative rather than any real run's actual (often smaller) batch size. Deliberately does not reflect the three things composed at runtime from a live run — the corpus digest, the persistent per-directory memory file's content (Z5, `.organizer/memory.md`), and the user's own pre-analysis steering instructions — since this view is target-free and must work before any directory has been analyzed
- `_resolve_analyzer_batch_size(settings) -> int` (Z3, #66) — the configured analysis batch size, for prompt text and `_analyze_new_documents`'s batch loop; falls back to the default `10` on any lookup failure (e.g. a bare `MagicMock` settings stub in tests, several of which pass one), mirroring `_resolve_quarantine_name`'s defensive pattern above
- `_load_memory(settings) -> str` (Z5) — reads the persistent per-directory memory file via `server.memory.read_memory(settings.memory_path, settings.max_snippet_chars)` for injection into `run_agent_loop`'s fresh-run seed message; never raises (defends against a bare mock `settings`, same pattern as `_resolve_analyzer_batch_size`) and strips any literal `_UNTRUSTED_CONTENT_BEGIN`/`_UNTRUSTED_CONTENT_END` delimiter text from the result first — a delimiter-forgery guard, so a poisoned agent-written note can't forge a fake "end of untrusted content" boundary that would make later real document text read as trusted
...
- `_handle_cost_approval(*, doc_count, already_analyzed, estimated_tokens, settings, on_event, on_cost_approval_needed) -> bool` (O8/P6, `already_analyzed` added P8) — relocated from a mid-loop tool-call interception to a one-time gate run once before `_analyze_new_documents` processes any new documents; emits a `"cost_estimate"` `AgentEvent` with `data={"new": doc_count, "already_analyzed": already_analyzed, "estimated_tokens": estimated_tokens, "batch_size": batch_size}`, where `batch_size = _resolve_analyzer_batch_size(settings)` (Z3; previously the fixed `_ANALYZER_BATCH_SIZE` constant) (from `_new_docs_cost_estimate`, new-docs-only, plus the known-doc count; `batch_size` added U5 — previously the real batch size never reached `data` at all, so both UIs' cost dialogs always displayed a hardcoded 10 regardless of the configured value, and the `summary` string interpolated a literal `10` rather than the resolved value), and — unless `approval_mode == "never"` or no callback is wired — awaits `on_cost_approval_needed`. Returns `True` if analysis should proceed; the event is always emitted, even when auto-approved, for observability
...
**Role:** Factory function for the OpenAI-compatible client. As of Y4 (GH #61), branches to Azure OpenAI's own client class when the configured endpoint looks like Azure, instead of sending a plain `AsyncOpenAI` request with an injected `api-version` query parameter — the old approach sent the wrong auth header and URL shape for Azure and broke a real user's config. As of Z2 (#67), also provides a self-reloading client so an in-flight run can recover from a bad LLM config (wrong model name, HTTP 429) without a full telcontar restart.

**Key type:** `AzureTarget` — frozen dataclass `{endpoint, deployment, api_version}`; `endpoint` is the bare resource root (no path), what `AsyncAzureOpenAI` expects.

**Key functions:**
- `resolve_azure_target(base_url, model, api_version) -> AzureTarget | None` — returns an `AzureTarget` when `base_url` looks like Azure OpenAI, else `None` (meaning: use the plain `AsyncOpenAI` path). Azure is detected by an explicit `api_version` OR an `*.azure.com` host, **except** when the path ends in `/openai/v1` (Azure's newer, genuinely OpenAI-compatible surface) — that combination always takes the plain-client path, since injecting a dated `api-version` alongside `/openai/v1` is exactly what produced the "API version not supported" failure in issue #61. Accepts four base-URL shapes: a bare resource root, `.../openai`, `.../openai/deployments/<name>` (the URL's own deployment segment wins over `LLM_MODEL` when they disagree), and `.../openai/v1`. When Azure is detected via hostname with no explicit `api_version`, falls back to `host/configflow.py`'s `AZURE_API_VERSION` constant.
- `make_client(settings) -> AsyncOpenAI` — builds an `httpx.AsyncClient` via `host/llmlog.py`'s `build_http_client` (Y5, always wired in) and, based on `resolve_azure_target`, either constructs an `AsyncAzureOpenAI` (with `azure_endpoint`/`azure_deployment`/`api_version` from the resolved `AzureTarget`) or a plain `AsyncOpenAI` (`base_url`/`api_key`). Either way, logs the resolved client shape once via `llmlog.log_client`. Both constructors pass `max_retries=4` (Z2; previously the SDK default), so a transient failure (e.g. HTTP 429) gets a few in-SDK retries before it ever surfaces as a run error.
- `make_reloading_client(settings, *, on_reload=None) -> AsyncOpenAI` (Z2) — builds a `ReloadingClient` and returns it `typing.cast` to `AsyncOpenAI`; the cast is documented in its own docstring as "the one deliberate lie in this module," since `ReloadingClient` only actually implements `.chat.completions.create`, the one surface `host/agent.py` ever calls on an `llm` object. `on_reload(message: str, new_model: str)`, if given, is invoked after a successful reload with a message naming the new model and the endpoint's hostname — never the API key.

**Key class:** `ReloadingClient` (Z2) — duck-types `AsyncOpenAI` for `.chat.completions.create`. Built via `make_client` like any other client, then snapshots `config_revision()` (`config/settings.py`). Every `create()` call first awaits `ensure_current()`: a no-op if the revision is unchanged (the common path, one int compare); on a changed revision, re-checks under an `asyncio.Lock` (double-checked locking, in case two calls raced), reloads `Settings` via `asyncio.to_thread(load_settings)` (off the event loop, since loading can hit the OS keyring), diffs only `_RELOADABLE_FIELDS` (`llm_base_url`, `llm_api_key`, `llm_model`, `llm_api_version`), `setattr`s the changed ones onto the *same* `Settings` object the caller passed in (mutated in place, never swapped), rebuilds the client via `make_client`, closes the superseded client (`contextlib.suppress`-guarded), and — if anything actually changed — calls `on_reload`. A failed reload (bad new config, keyring hiccup) is caught and swallowed: the existing client is kept, nothing raises into the run, and the broken config isn't retried again until the revision changes once more. `approval_mode` and every path field are deliberately excluded from `_RELOADABLE_FIELDS` — only the LLM endpoint fields ever hot-swap.

**Design note:** Provider-specific code is now needed — the module branches on Azure vs. non-Azure to build the right client class, since a plain `AsyncOpenAI` cannot address Azure's endpoint/auth shape outside its `/openai/v1` compatibility surface.

---

### `host/llmlog.py`

**Role:** Always-on, redacted debug log (Y5, GH #60) of every outbound LLM HTTP call, to help diagnose corporate/Azure connectivity issues (e.g. issue #61's kind of misconfiguration) without a reproduce-with-a-flag round trip. Writes to `settings.llm_debug_log_path` (`.organizer/llm-debug.jsonl`, rebased per-target like the other `.organizer` paths). Deliberately does not enable the OpenAI SDK's own `OPENAI_LOG`, which would dump full request bodies — including extracted document text — to whatever handler is configured; this module only ever logs counts, statuses, and (on an HTTP >=400 response) the provider's own truncated error body, none of which carries document content.

**Key type:** `LLMLogEntry` — frozen dataclass `{ts, kind, detail}`; `kind` is one of `"client"`, `"request"`, `"response"`, `"error"`. `.new(kind, detail)` stamps `ts` (UTC ISO-8601); `.to_dict()` serializes for JSONL.

**Key functions:**
- `append(path, entry)` — appends one entry as a JSONL line, creating parent dirs; never raises (a debug log must never break the run it's diagnosing)
- `all_entries(path)` — reads back every logged entry, oldest first; `[]` if the file is missing or unreadable
- `log_client(path, *, client_class, endpoint, api_version, deployment, model, auth_header)` — logs the resolved client shape once, right after `make_client` builds it: `client_class` (`"AsyncAzureOpenAI"` or `"AsyncOpenAI"`), `endpoint`, `api_version`, `deployment`, `model`, and which header carries the credential (`"api-key"` for Azure, `"Authorization"` otherwise)
- `build_http_client(log_path) -> httpx.AsyncClient` — returns an `httpx.AsyncClient` wired to log every LLM HTTP call, preserving the OpenAI SDK's own transport defaults explicitly (a bare `httpx.AsyncClient()` defaults to a 5s timeout, which would break every long analyze call). Its `request`/`response` event hooks log a `"request"` entry (method, redacted URL, message count parsed from the request body, byte count) and a `"response"` entry (status, duration, request id pulled from `x-request-id`/`apim-request-id`/`x-ms-request-id`, redacted URL, and — on status >= 400 — the truncated response body). Its transport is wrapped in `_LoggingTransport`, which logs an `"error"` entry for transport-level failures (connect errors, timeouts, TLS/proxy errors) — the one failure class httpx event hooks never see, since they only fire once a request/response stage actually completes — then re-raises

**Design note:** URLs are redacted before logging (`_redact_url`) — any query value under a credential-bearing key (`api-key`, `key`, `code`, `subscription-key`) is masked, while other query params (notably `api-version`, the whole diagnostic point) survive.
...
**Role:** UI-agnostic path/discovery helpers used by the web UI. Extracted from `host/app.py` (Phase 18 S1) with zero behaviour change.

**Key functions:**

| Function | Description |
|---|---|
| `find_organizer_root(start) -> Path \| None` | (P2) Walks up from `start` to find the nearest directory containing `.organizer` — a folder picked for Query may be a subfolder of what was actually organized. `None` if none is found before the filesystem root. |
| `resolve_journal_path(target)` / `resolve_plans_dir(target)` / `resolve_registry_path(target)` / `resolve_graph_path(target)` / `resolve_events_path(target)` / `resolve_sessions_dir(target)` | Each resolves the corresponding `Settings().for_target(target)` path/dir for `target` — thin, settings-backed lookups used wherever the web UI needs one of these paths outside a live run. |
| `resolve_memory_path(target) -> Path` (Z5) | Resolves the persistent per-directory memory file (`Settings().for_target(target).memory_path`) for `target`, the same way its siblings above resolve the other per-target paths. |
| `memory_status_text(target) -> str` (Z5) | One-line starter-pane status: `"Using notes from memory.md — N characters"` if a memory file already exists for `target`, `""` if there is none or on any error (a display nicety, never a safety guard — makes the eventual prompt injection visible before the run starts, since a hostile corpus could in principle ship its own `memory.md`). |
| `quarantine_basename() -> str` | (P2) Basename of the configured quarantine dir, for discovery-hiding in `directory_overview`. Falls back to `"_quarantine"` on any settings error. |
| `directory_overview(target, max_entries=5000) -> str` | (L3) Code-generated, deterministic one-glance directory summary (file/subfolder counts, most common extensions) — names and structure only, no file content, no LLM call. Bounded by `max_entries` so a huge tree can't stall the UI. Shown on the organize starter pane before any run starts. |
| `reveal_in_file_manager(path) -> bool` | (X5) Fire-and-forget `subprocess.Popen` opening the OS file manager with `path` selected (Explorer `/select` on Windows, `open -R` on macOS, falls back to opening the parent folder on Linux — no portable "select a file" verb there). Never raises, never waits on/checks the child process's exit code (Windows Explorer exits 1 even on success). |
| `is_op_out_of_scope(op, target) -> bool` | Best-effort, advisory-only UI check: does `op`'s `src` resolve outside `target`? The server's own `check_within_root` (M2) is the real enforcement boundary; this only makes the risk visible to the approver. Any resolution error (missing src, bad path) reads as in-scope rather than raising. |

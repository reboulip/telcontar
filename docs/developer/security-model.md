...
- **Every mutation is now plan-gated (S1, remediated)**: there is no MCP tool that
  ...
  agent-callable at all — it is a web-UI-only user action. This invariant held through
  Z5's persistent per-directory memory: `propose_memory_note` was deliberately designed
  as a plan op (staged, approved, applied only by `execute_plan`, self-journaling like
  `archive_document`/`compress_quarantine`) rather than a direct-write tool, specifically
  so this "no write path outside `execute_plan`" property would remain true — there is
  no direct-write memory tool.

...
Not yet reachable from a live organize run (the analyzer isn't wired into
`run_agent_loop` yet).
...
| Registry / graph / events | `record_document`, `record_document_batch` (O2), `rehome_documents` (P4), `get_*`, `list_*`, `build_graph`, `create_event`, … | No (metadata only) |

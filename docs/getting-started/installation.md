...
`telcontar` also accepts `--web` (launches the web UI in a local browser tab instead of the TUI — has its own setup wizard, settings view, journal/undo view, and read-only query mode at parity with the TUI's, but still lacks a startup screen offering a direct Query entry point without organizing first) and `--target PATH` (only meaningful with `--web`; jumps straight to a run for `PATH` instead of showing the web UI's directory picker).

...
`telcontar` also accepts `--web` (launches the web UI in a local browser tab instead of the TUI — has its own setup wizard and settings view at parity with the TUI's, but still lacks query mode and the journal/undo view) and `--target PATH` (only meaningful with `--web`; jumps straight to a run for `PATH` instead of showing the web UI's directory picker).

...
!!! note "The web UI is the default; this page walks through the Textual TUI"
    Bare `telcontar` (no flags) now opens the web UI in its own native window instead (via `pywebview`, Windows only — falls back to a browser tab automatically, with a warning, if that's unavailable) — the same organize experience walked through on this page, but with its own first-run setup wizard at `/setup`, a settings view at `/settings` (reachable from every screen via the sidebar), a journal/undo view, and a startup screen with direct Organize/Query/Settings entry points at parity with the TUI's. Pass `--tui` to launch the Textual TUI walkthrough shown on this page, `--target PATH` to skip the landing page's directory picker and jump straight into a run for `PATH` (ignored together with `--tui`), and `--browser` to always use the browser tab instead of the native window (also ignored together with `--tui`).

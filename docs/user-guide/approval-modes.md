...
- **A target-layout preview**, when the plan has any move/quarantine destinations: a folder tree built from the plan's ops, with each folder's purpose note (set by the agent via `set_plan_folder_notes`) shown beside it — folders without a note appear as bare tree nodes. Rename-only plans show no tree. When any folder notes are present, a matching disclaimer ("Folder notes are model-generated — not verified fact.") appears above the tree — the tree structure itself is derived from the plan's own op destinations, but the prose notes beside each folder are the agent's commentary, not verified fact
- **Each proposed operation** is listed as a checked checkbox — an `update_file` op that will replace an existing file (`overwrite=True`) is labelled with a subtle `(overwrite)` marker so you aren't blindsided by a collision-safe write turning into a replace, and an op whose source resolves outside the directory you're organizing is labelled with an equally subtle `(outside target)` marker — a best-effort visual cue, not the security boundary itself (that's the server's own path-confinement guard)

...
- **Each proposed operation** is listed as a checked checkbox — an `update_file` op that will replace an existing file (`overwrite=True`) is labelled with a subtle `(overwrite)` marker so you aren't blindsided by a collision-safe write turning into a replace

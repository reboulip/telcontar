...
A **GPT-5–authored narrative** summary describing:

- What the directory contains (types of documents, time range, key people)
- What was organized (how many files renamed, moved, quarantined)
- Any notable findings (duplicates detected, modified versions identified)

The content is fully generated by the model; telcontar's `write_summary` tool simply persists the prose the agent composes.

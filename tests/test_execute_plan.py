...
class TestExecutePlanHappyPath:
    ...
    def test_quarantine_op_succeeds(self, tmp_path: Path, plans_dir: Path, journal_path: Path) -> None:
    ...
    def test_empty_plan_completes_as_done(self, plans_dir: Path, journal_path: Path, approved_plan: Plan) -> None:
        result = execute_plan(approved_plan.plan_id, plans_dir, journal_path)
        assert result["state"] == "done"
        assert result["ops_completed"] == 0

    def test_multiple_ops_all_succeed(self, tmp_path: Path, plans_dir: Path, journal_path: Path) -> None:
        dst_dir = tmp_path / "dest"
        dst_dir.mkdir()
...
class TestExecutePlanStateTransitions:
    ...
    def test_transitions_through_executing_to_done(self, tmp_path: Path, plans_dir: Path, journal_path: Path) -> None:
        src = tmp_path / "f.txt"
        src.write_text("x")
        p = Plan.new()
        p.transition("approved")
    ...
    def test_partial_failure_ends_in_failed(self, tmp_path: Path, plans_dir: Path, journal_path: Path) -> None:
...
class TestExecutePlanRetry:
    def test_retries_on_oserror_then_succeeds(self, tmp_path: Path, plans_dir: Path, journal_path: Path) -> None:
        src = tmp_path / "file.txt"
        src.write_text("x")
        dst_dir = tmp_path / "dst"
        dst_dir.mkdir()
    ...
    def test_exhausts_retries_on_persistent_oserror(self, tmp_path: Path, plans_dir: Path, journal_path: Path) -> None:
        src = tmp_path / "file.txt"
        src.write_text("x")
        dst_dir = tmp_path / "dst"
        dst_dir.mkdir()
    ...
    def test_file_not_found_does_not_retry(self, tmp_path: Path, plans_dir: Path, journal_path: Path) -> None:
        p = Plan.new()
        p.transition("approved")
...
class TestExecutePlanHardStop:
    def test_hard_stop_after_four_failures(self, tmp_path: Path, plans_dir: Path, journal_path: Path) -> None:
        p = Plan.new()
        p.transition("approved")
        for i in range(6):
    ...
    def test_hard_stop_writes_journal_entry(self, tmp_path: Path, plans_dir: Path, journal_path: Path) -> None:
        p = Plan.new()
        p.transition("approved")
        for i in range(5):
    ...
    def test_hard_stop_leaves_unprocessed_ops_pending(self, tmp_path: Path, plans_dir: Path, journal_path: Path) -> None:
        good = tmp_path / "good.txt"
        good.write_text("x")
        dst_dir = tmp_path / "dst"
        dst_dir.mkdir()
    ...
    def test_three_failures_does_not_trigger_hard_stop(self, tmp_path: Path, plans_dir: Path, journal_path: Path) -> None:
        p = Plan.new()
        p.transition("approved")
        for i in range(3):
            p.ops.append(PlanOp.new("rename", str(tmp_path / f"m_{i}.txt"), f"n_{i}.txt"))
...
class TestExecutePlanJournal:
    def test_journals_each_successful_op(self, tmp_path: Path, plans_dir: Path, journal_path: Path) -> None:
        dst_dir = tmp_path / "dst"
        dst_dir.mkdir()
        files = [tmp_path / f"f{i}.txt" for i in range(3)]
        for f in files:
    ...
    def test_failed_op_not_journaled(self, tmp_path: Path, plans_dir: Path, journal_path: Path) -> None:
        p = Plan.new()
        p.transition("approved")
        p.ops.append(PlanOp.new("rename", str(tmp_path / "missing.txt"), "new.txt"))
        save(p, plans_dir)
    ...
    def test_journal_entry_has_required_fields(self, tmp_path: Path, plans_dir: Path, journal_path: Path) -> None:
        src = tmp_path / "file.txt"
        src.write_text("x")
        p = Plan.new()
        p.transition("approved")
    ...
    def test_idempotent_skips_completed_ops(self, tmp_path: Path, plans_dir: Path, journal_path: Path) -> None:
        src = tmp_path / "file.txt"
        src.write_text("x")
        p = Plan.new()
        p.transition("approved")
